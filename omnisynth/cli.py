"""
CLI entry point for OmniSynth.

Commands
--------
OmniSynth dashboard [--config PATH] [--port PORT]
    Launch the Streamlit interactive dashboard.

OmniSynth evaluate --real PATH --synth PATH [PATH ...] [--output PATH]
    Run evaluation headlessly and write results to a JSON file.

OmniSynth precompute --config PATH --output PATH [--groups bivariate multivariate]
...
def _cmd_evaluate(args):
    ...
    from omnisynth.utils.data_utils import load_config, eval_config_from_dict, weights_from_dict
...
def _cmd_precompute(args):
    ...
    from omnisynth.evaluation.fidelity import evaluate_fidelity
    from omnisynth.evaluation.missingness import evaluate_missingness
    from omnisynth.utils.data_utils import load_config, eval_config_from_dict
    from omnisynth.utils.precomputed_io import save_precomputed
...
def _cmd_meta_eval(args):
    from omnisynth.meta_eval.config import load_meta_eval_config
    from omnisynth.meta_eval.runner import run_meta_eval, save_meta_eval_results
